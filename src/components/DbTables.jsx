...
const TableDropdown = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      for (const rowIndex of selectedRows) {
        ...
        console.log("Saving row:", row); // Check the data you're saving
        ...
        if (row.id && Number.isInteger(row.id)) {
          ...
          const maxId = Math.max(...tableData.map((r) => (r.id ? r.id : 0)));
          row.id = maxId + 1;
          console.log("Adding new row with generated ID:", row.id);
          await axios.post("http://localhost:5000/api/addNewRecord", {
            tableName: selectedTable,
            fieldNames: Object.keys(row).join(","),
            fieldValues: Object.values(row).join(","),
  ...
  return (
    <div className="flex flex-col w-full p-6 bg-gray-50">
      ...
      {selectedTable && (
        <div className="w-full overflow-x-auto shadow-md bg-white rounded-md p-4">
